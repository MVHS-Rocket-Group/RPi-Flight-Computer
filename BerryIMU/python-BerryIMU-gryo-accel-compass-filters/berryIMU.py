...
# Median filter table size for accelerometer. Higher = smoother but a longer delay
ACC_MEDIANTABLESIZE = 9
# Median filter table size for magnetometer. Higher = smoother but a longer delay
...
while True:
    ...
    if(IMU.LSM9DS0):
        magYcomp = MAGx*math.sin(roll)*math.sin(pitch)+MAGy * \
            math.cos(roll)-MAGz*math.sin(roll)*math.cos(pitch)  # LSM9DS0
    else:
        magYcomp = MAGx*math.sin(roll)*math.sin(pitch)+MAGy * \
            math.cos(roll)+MAGz*math.sin(roll)*math.cos(pitch)  # LSM9DS1
